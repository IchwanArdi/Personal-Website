...
const MobileMenu = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      {/* Mobile Menu */}
      <div
        className={`mobile-menu lg:hidden fixed top-0 right-0 h-screen w-80 backdrop-blur-md shadow-2xl z-50 transition-all duration-500 ease-out border-l ${
          isDarkMode ? 'bg-black/95 border-gray-800/30' : 'bg-white/95 border-gray-300/50'
      ...
      {/* Enhanced Backdrop */}
      {isOpen && <div className={`lg:hidden fixed inset-0 backdrop-blur-sm z-40 transition-all duration-500 animate-in fade-in ${isDarkMode ? 'bg-black/70' : 'bg-gray-900/50'}`} onClick={handleBackdropClick} />}
    </>
  );
};
